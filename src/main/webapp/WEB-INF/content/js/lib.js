...
function setMainTable(json) {
    ...
    var mainPageChooseBar = document.getElementById("main_page_choose_bar");
    if (table != null) {
        table.remove();
        mainPageChooseBar.remove();
        button.remove();
    }
    table = document.createElement("table");
    button = document.createElement("bUtton");
    var buttonText = document.createTextNode("添加" + document.getElementById("main_title").firstChild.nodeValue.split("管理")[0]);
    button.appendChild(buttonText);
    button.setAttribute("id", "main_btn");
    button.setAttribute("type", "button");
    button.setAttribute("onclick", "openCreator()");
    ...
    var a = 0;
    for (var childTitle in title) {
        var th = document.createElement("th");
        trth.appendChild(th);
        th.setAttribute("name", childTitle);
        var thText = document.createTextNode(title[childTitle]);
